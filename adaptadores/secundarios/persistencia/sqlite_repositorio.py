...
class SQLiteRepositorio(RepositorioClima):
    ...
    def _conectar(self):
        ...
        # Verificar activacion de claves foraneas
        cursor = conn.cursor()
        cursor.execute("PRAGMA foreign_keys;")
        fk_status = cursor.fetchone()[0]
        if fk_status != 1:
            raise RuntimeError(
                "No se pudieron activar las claves foráneas (PRAGMA foreign_keys = ON)")
    ...
    def _inicializar(self):
        ...
        cursor = conn.cursor()
        try:
            # 1. Crear tabla de control de versiones de migraciones
            cursor.execute("""
            ...
            # 2. Verificar si la migración de versión 1 ya ha sido aplicada
            cursor.execute(
            ...
            if not migration_applied:
                ...
                # Buscar tablas existentes para migrar datos
                cursor.execute("SELECT name FROM sqlite_master WHERE type='table' "
                               "AND name IN ('ciudades', 'clima_actual', 'clima_pronostico', 'historial_consultas', "
                               "'analisis_clima', 'log_auditoria', 'recomendaciones_ia')")
                existing_old_tables = [row[0] for row in cursor.fetchall()]

                needs_migration = False
                if 'clima_actual' in existing_old_tables:
                    # Verificar si tiene las columnas del esquema antiguo (ej. 'temp')
                ...
                if needs_migration:
                    # Eliminar triggers antiguos para evitar interferencias
                    cursor.execute(
                        "DROP TRIGGER IF EXISTS trg_validar_humedad;")
                    cursor.execute(
                        "DROP TRIGGER IF EXISTS trg_auditar_consulta;")
                    cursor.execute("DROP TRIGGER IF EXISTS trg_log_analisis;")

                    # Renombrar tablas para preservar los datos
                    for t in existing_old_tables:
                        cursor.execute(f"ALTER TABLE {t} RENAME TO old_{t};")

                # Crear el nuevo esquema en español con restricciones CHECK robustas y FKs con nombres explícitos
                cursor.execute("""
                ...
                cursor.execute("""
                    CREATE TABLE clima_actual (
                        ...
                        fecha_guardado TEXT NOT NULL DEFAULT (datetime('now','localtime'))
                            CHECK(fecha_guardado GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9] 
                               [0-9][0-9]:[0-9][0-9]:[0-9][0-9]'),
                        CONSTRAINT fk_clima_actual_ciudad FOREIGN KEY (ciudad) REFERENCES ciudades(nombre) 
                               ON DELETE CASCADE
                    );
                """)
                ...
                cursor.execute("""
                    CREATE TABLE clima_pronostico (
                        ...
                        fecha_guardado TEXT NOT NULL DEFAULT (datetime('now','localtime'))
                            CHECK(fecha_guardado GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9] 
                               [0-9][0-9]:[0-9][0-9]:[0-9][0-9]'),
                        CONSTRAINT fk_clima_pronostico_ciudad FOREIGN KEY (ciudad) REFERENCES 
                               ciudades(nombre) ON DELETE CASCADE,
                ...
                cursor.execute("""
                    CREATE TABLE historial_consultas (
                        ...
                        fecha_consulta TEXT NOT NULL DEFAULT (datetime('now','localtime'))
                            CHECK(fecha_consulta GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9] 
                               [0-9][0-9]:[0-9][0-9]:[0-9][0-9]'),
                        ciudad TEXT NOT NULL,
                        temperatura REAL NOT NULL CHECK(temperatura BETWEEN -50 AND 60),
                        estado TEXT NOT NULL CHECK(estado IN ('URBAN','NORMAL','FAVORABLE','RIESGOSO')),
                        CONSTRAINT fk_historial_consultas_ciudad FOREIGN KEY (ciudad) REFERENCES ciudades(nombre) 
                               ON DELETE CASCADE
                    );
                """)
                ...
                cursor.execute("""
                    CREATE TABLE analisis_clima (
                        ...
                        fecha_analisis TEXT NOT NULL DEFAULT (datetime('now','localtime'))
                            CHECK(fecha_analisis GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9] 
                               [0-9][0-9]:[0-9][0-9]:[0-9][0-9]'),
                        CONSTRAINT fk_analisis_clima_clima_actual FOREIGN KEY (clima_actual_id) 
                               REFERENCES clima_actual(id) ON DELETE CASCADE
                    );
                """)
                ...
                # Migrar datos desde tablas antiguas a las nuevas
                if needs_migration:
                    # ciudades
                    cursor.execute("""
                        INSERT INTO ciudades (nombre, latitud, longitud, es_agricola)
                    ...
                    # clima_pronostico
                    cursor.execute("""
                        INSERT OR IGNORE INTO clima_pronostico (ciudad, fecha, temperatura_maxima, temperatura_minima, precipitacion, humedad, fecha_guardado)
                        SELECT ciudad, substr(date, 1, 10), temp_max, temp_min, precipitation, humidity, fecha_guardado FROM old_clima_pronostico;
                    """)
                    ...
                    # analisis_clima migra el ultimo analisis
                    cursor.execute("""
                        ...
                        SELECT
                            ...
                            ac.condiciones,
                            ac.mensaje,
                            COALESCE(ac.recomendacion_ia, ''),
                            ac.fecha_consulta
                        FROM old_analisis_clima ac
                        JOIN clima_actual ca ON ca.ciudad = ac.ciudad
                        WHERE ac.id = (
                            SELECT MAX(ac2.id)
                            FROM old_analisis_clima ac2
                            WHERE ac2.ciudad = ac.ciudad
                        );
                    """)
                    ...
                    cursor.execute("""
                        ...
                        SELECT
                            tabla,
                            CASE WHEN operacion IN ('INSERT','UPDATE','DELETE') THEN operacion ELSE 'INSERT' END,
                            registro_id,
                            detalle,
                            fecha
                        FROM old_log_auditoria;
                    """)
                ...
                # Cargar ciudades desde configuracion settings en caso de base de datos nueva
                ...
                cursor.execute("""
                    ...
                    VALUES (1, 'Creación de esquema unificado en español con mappers, CHECK y purga automatizada.')
                """)
                conn.commit()
            ...
            migration2_applied = cursor.fetchone()[0] > 0

            if not migration2_applied:
                cursor.execute("BEGIN TRANSACTION;")
                cursor.execute("DROP VIEW IF EXISTS vista_resumen_agricola;")
                cursor.execute("""
                    CREATE VIEW vista_resumen_agricola AS
                    SELECT 
                        c.nombre AS ciudad,
                        c.latitud,
                        c.longitud,
                        ...
                        ca.fecha_guardado
                    FROM ciudades c
                    JOIN clima_actual ca ON c.nombre = ca.ciudad
                    LEFT JOIN analisis_clima ac ON ca.id = ac.clima_actual_id
                    WHERE c.es_agricola = 1;
                """)

                cursor.execute("DROP VIEW IF EXISTS vista_estadisticas_clima;")
                cursor.execute("""
                    CREATE VIEW vista_estadisticas_clima AS
                    SELECT 
                        ciudad,
                        AVG(temperatura_maxima) AS temperatura_maxima_promedio,
                        AVG(temperatura_minima) AS temperatura_minima_promedio,
                        SUM(precipitacion) AS precipitacion_total,
                        AVG(humedad) AS humedad_promedio,
                        COUNT(id) AS dias_pronosticados
                    FROM clima_pronostico
                    GROUP BY ciudad;
                """)
                cursor.execute("""
                    INSERT INTO _migrations (version, description)
                    VALUES (2, 'Creación de las vistas vista_resumen_agricola y vista_estadisticas_clima adaptadas al nuevo esquema')
                """)
                conn.commit()
        finally:
            conn.close()
    ...
    def guardar_clima(self, ciudad: str, datos: DatosClima) -> None:
        ...
        try:
            ...
            # Guardar o actualizar clima_actual
            cursor.execute("""
                INSERT INTO clima_actual (ciudad, temperatura, humedad, precipitacion, fecha_guardado)
            ...
            # Reemplazar pronostico para la ciudad
            cursor.execute(
                "DELETE FROM clima_pronostico WHERE ciudad = ?", (ciudad,))
            for f in datos.pronostico:
                cursor.execute("""
                    INSERT INTO clima_pronostico
                    (ciudad, fecha, temperatura_maxima, temperatura_minima, precipitacion, humedad, fecha_guardado)
                    VALUES (?,?,?,?,?,?, datetime('now','localtime'))
                """, (ciudad, f.fecha, f.temperatura_max, f.temperatura_min, f.precipitacion, f.humedad))

            #
            # Mantener maximo 10 dias en clima_pronostico
            cursor.execute(
                "DELETE FROM clima_pronostico WHERE date(fecha) < date('now', '-10 days')")
            # Mantener maximo 90 dias en historial_consultas
            cursor.execute(
                "DELETE FROM historial_consultas WHERE date(fecha_consulta) < date('now', '-90 days')")
            # Mantener maximo 365 dias en log_auditoria
            cursor.execute(
                "DELETE FROM log_auditoria WHERE date(fecha) < date('now', '-365 days')")
    ...
    def cargar_clima(self, ciudad: str) -> Optional[DatosClima]:
        ...
        try:
            cursor.execute(
                "SELECT temperatura, humedad, precipitacion FROM clima_actual WHERE ciudad = ?",
                (ciudad,)
            )
            row = cursor.fetchone()
            if not row:
            ...
            actual = MedicionActual(
                temperatura=row[0], humedad=row[1], precipitacion=row[2])
            ...
            pronostico = [PronosticoDia(fecha=r[0], temperatura_max=r[1], temperatura_min=r[2],
                                        precipitacion=r[3], humedad=r[4])
    ...
    def guardar_analisis(self, ciudad: str, analisis: AnalisisLocal, rec_ia: str) -> None:
        ...
        try:
            ...
            # Obtener id de clima_actual
            cursor.execute(
                "SELECT id FROM clima_actual WHERE ciudad = ?", (ciudad,))
            row = cursor.fetchone()
            if not row:
                raise ValueError(
                    f"No existe clima actual registrado para la ciudad: {ciudad}")
            clima_actual_id = row[0]

            # Insertar o actualizar analisis_clima (evitando borrar IDs)
            cursor.execute("""
                INSERT INTO analisis_clima (clima_actual_id, estado, condiciones, mensaje, 
            ...
            """, (clima_actual_id, analisis.estado,
                  ",".join(
                      analisis.condiciones) if analisis.condiciones else "NINGUNA",
    ...
    def cargar_analisis(self, ciudad: str) -> Optional[dict]:
        ...
        try:
            cursor.execute("""
                ...
                FROM analisis_clima ac
                JOIN clima_actual ca ON ac.clima_actual_id = ca.id
                WHERE ca.ciudad = ?
                ORDER BY ac.fecha_analisis DESC LIMIT 1
            """, (ciudad,))
            row = cursor.fetchone()
            if not row:
                return None
    ...
    def guardar_consulta_historial(self, ciudad: str, temperatura: float, estado: str) -> None:
        ...
        try:
            cursor.execute("""
                INSERT INTO historial_consultas (ciudad, temperatura, estado)
                VALUES (?, ?, ?)
    ...
    def historial(self, limite: int = 5) -> List[ConsultaHistorial]:
        ...
        try:
            ...
            return [
                ConsultaHistorial(
                    fecha=r[0], ciudad=r[1], temperatura=r[2], estado=r[3])
                for r in cursor.fetchall()
            ]
        finally:
            conn.close()
